...
// Tự động tạo bảng matches nếu chưa tồn tại
try {
    ...
    // Cập nhật thêm cột tournament_name cho bảng matches nếu chưa có (cho trường hợp bảng đã tồn tại từ trước)
    try {
        $db->exec("ALTER TABLE matches ADD COLUMN tournament_name VARCHAR(255) AFTER id");
    } catch (Exception $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($action == 'add' || $action == 'edit') {
        try {
            ...
            if ($action == 'add') {
                $stmt = $db->prepare("INSERT INTO matches (tournament_name, game_type, team1_name, team1_logo, team2_name, team2_logo, match_time, status, score_team1, score_team2, stream_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute($data);
                $message = '<div class="alert success">Match added successfully!</div>';
            } else {
                $data[] = $_GET['id'];
                $stmt = $db->prepare("UPDATE matches SET tournament_name=?, game_type=?, team1_name=?, team1_logo=?, team2_name=?, team2_logo=?, match_time=?, status=?, score_team1=?, score_team2=?, stream_link=? WHERE id=?");
                $stmt->execute($data);
...
// Lấy danh sách tên giải đấu để lọc
$stmt = $db->query("SELECT DISTINCT tournament_name FROM matches WHERE tournament_name IS NOT NULL AND tournament_name != '' ORDER BY tournament_name");
$tournamentList = $stmt->fetchAll(PDO::FETCH_COLUMN);

// Lấy danh sách matches
$tournamentFilter = $_GET['tournament_filter'] ?? '';
...
<head>
    ...
    <title>Tournaments Management - <?php echo SITE_NAME; ?></title>
    ...
    <div class="admin-container">
        ...
        <div class="main-content">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="tournament_name" value="<?php echo $match['tournament_name'] ?? ''; ?>" placeholder="e.g. Worlds 2023">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" name="stream_link" value="<?php echo $match['stream_link'] ?? ''; ?>" placeholder="https://twitch.tv/...">
                        </div>
            ...
            <div class="table-container">
                <!-- Filter Form -->
                <div class="search-filter" style="margin: 0 0 20px 0; padding: 15px; box-shadow: none; border-bottom: 1px solid #eee;">
                    <form method="GET" class="search-form">
                        <select name="tournament_filter" onchange="this.form.submit()">
                            <option value="">-- All Tournaments --</option>
                            <?php foreach ($tournamentList as $tName): ?>
                                <option value="<?php echo htmlspecialchars($tName); ?>" <?php echo $tournamentFilter === $tName ? 'selected' : ''; ?>>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <strong><?php echo htmlspecialchars($m['tournament_name'] ?? 'N/A'); ?></strong>
                                <br><span class="badge"><?php echo strtoupper($m['game_type']); ?></span>
                            </td>
                            <td>
